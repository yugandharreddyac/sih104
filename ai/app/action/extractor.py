...
class RequestedActionExtractor:
    def __init__(self):
        ...
        self.action_rules = [
            ...
            (re.compile(r'\b(transfer|send|wire|pay)\s+(\$|₹|rs\.?|inr|money|funds|\d+|[a-z]+\s+(thousand|hundred|crore|lakh|rupees))\b|\b(transfer|send|wire|pay)\b.*\b(rupees|funds|money)\b', re.I), ActionType.TRANSFER_FUNDS, "Transfer financial funds", True),
            ...
            (re.compile(r'\b(install|download|run)\s+(?:[a-z]+\s+)*(anydesk|teamviewer|quicksupport|software|app)\b', re.I), ActionType.INSTALL_REMOTE_SOFTWARE, "Install remote desktop / access software", True),
    ...
    def extract_action(self, text: str) -> RequestedActionResult:
        ...
        for pattern, action_type, desc, is_high_risk in self.action_rules:
            ...
            if m:
                matches.append((m.start(), action_type, desc, is_high_risk, m.group(0)))

        if matches:
            # Sort by earliest occurrence in speech
            matches.sort(key=lambda x: x[0])
            _, action_type, desc, is_high_risk, raw_match = matches[0]
            return RequestedActionResult(
                action_type=action_type,
                target_object=desc,
                is_high_risk=is_high_risk,
                confidence=0.92,
                raw_action_text_redacted=self.redactor.redact(raw_match)
            )
