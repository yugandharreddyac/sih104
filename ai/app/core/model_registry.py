    ...
    def initialize_defaults(cls):
        ...
        registered_list = [
            ...
            ModelMetadata(
                ...
                device=device,
                status=PipelineStatus.AVAILABLE,
                checksum_sha256="245eb5995cfffd74494862dee33da2b00c1c2579eb0c6703847784e9901ed458",
                ...
                known_limitations="Primary ECAPA-TDNN ONNX neural weights active on CPU with 192-dim embeddings; graceful fallback to deterministic DSP 64-band FFT filterbank with random projection (128-dim) if disabled or unavailable.",
                registered_at=now
            ),
            ModelMetadata(
                ...
                training_dataset="VoxCeleb 1 & 2 Multilingual Conversational Corpus",
                input_sample_rate=16000,
                inference_latency_ms_p50=25.0,
                known_limitations="Real ECAPA-TDNN ONNX neural inference active on CPU (192-dim). Requires 16 kHz mono audio (resampling applied automatically).",
                registered_at=now
            ),
            ModelMetadata(
