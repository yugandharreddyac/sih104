...
class EvidenceCompiler:
    def compile_soc_diagnostic(
        ...
        if overall_score is None:
            explanations.append("EVALUATION PENDING / INCONCLUSIVE: Multi-modal telemetry awaiting sufficient audio/conversational stream.")
        ...
        top_drivers = []
        if dimensions.credential_theft is not None and dimensions.credential_theft >= 70.0:
            top_drivers.append(f"Credential Harvesting ({dimensions.credential_theft:.0f}/100)")
        if dimensions.identity_impersonation is not None and dimensions.identity_impersonation >= 70.0:
            top_drivers.append(f"Identity Impersonation ({dimensions.identity_impersonation:.0f}/100)")
        if dimensions.social_engineering is not None and dimensions.social_engineering >= 70.0:
            top_drivers.append(f"Social Engineering Pressure ({dimensions.social_engineering:.0f}/100)")
        if dimensions.deepfake_synthetic is not None and dimensions.deepfake_synthetic >= 70.0:
            top_drivers.append(f"Synthetic Voice Clone ({dimensions.deepfake_synthetic:.0f}/100)")
        if dimensions.financial_fraud is not None and dimensions.financial_fraud >= 70.0:
