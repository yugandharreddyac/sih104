...
describe('Deterministic Policy Engine Unit Tests', () => {
  ...
  it('should enforce BLOCK_DISCLOSURE when requested_information is OTP', () => {
    const context = {
  ...
  it('should enforce REQUIRE_STEP_UP_VERIFICATION for unverified high-value wire transfers', () => {
    const context = {
  ...
  it('should block protected workflows when deepfake risk is HIGH and action risk is CRITICAL', () => {
    const context = {
      deepfake_risk: 'HIGH',
  ...
  it('should allow benign transactions with no matching blocking rules', () => {
    const context = {
