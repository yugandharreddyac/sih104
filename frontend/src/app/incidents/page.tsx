          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="soc-glass p-4 rounded-xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                ...
                <button onClick={fetchIncidents} className="p-1 text-slate-400 hover:text-white rounded">
              ...
              <div className="space-y-2">
                {incidents.map((inc) => (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-slate-500 font-mono flex items-center justify-between mt-2">
              ...
              {selectedIncident ? (
                <>
                  <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-amber-600/20 hover:bg-amber-600/30 text-amber-300 border border-amber-500/30 rounded-lg text-xs font-semibold"
                      >
                        Contain
                      </button>
                      <button
                        onClick={() => handleUpdateStatus(selectedIncident.id, 'RESOLVED')}
                        className="px-3 py-1.5 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/30 rounded-lg text-xs font-semibold"
                      >
                        Resolve
                      </button>
                      <button
                  ...
                  <div className="p-3.5 rounded-lg bg-slate-900/80 border border-slate-800 text-xs text-slate-200 leading-relaxed">
                    <span className="text-slate-400 font-mono block text-[10px] uppercase mb-1">Incident Summary</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs font-mono">
                    <div className="p-3 rounded-lg bg-slate-900/60 border border-slate-800">
                      <span className="text-slate-500 block text-[10px] uppercase mb-1.5">Triggered Policies</span>
                      {selectedIncident.triggeredPolicies?.map((pol: string, idx: number) => (
                        <div key={idx} className="text-indigo-300 font-medium">{pol}</div>
                      ))}
                    </div>
                    <div className="p-3 rounded-lg bg-slate-900/60 border border-slate-800">
                      <span className="text-slate-500 block text-[10px] uppercase mb-1.5">Actions Enforced</span>
